<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($real_name)) {
            ...
            $real_name_err = "";
            $real_name = test_input($real_name);
        }

        // Validate user address at sever side
        if (!checkSelect($distribution_hub)) {
            $distribution_hub_err = "Please select a distribution hub";
        } else if (!checkMinLength($distribution_hub, 5)) {
            $distribution_hub_err = "At least 5 characters is required";
        } else {
            $distribution_hub_err = "";
            $distribution_hub = test_input($distribution_hub);
        }
        
        // If all form fields satisfy the requirements, save the user data to the text file
        if ($username && $password && $real_name && $distribution_hub && !$is_duplicate) {
            $user_type = "type3";  //type 3 represents for shipper accounts
                            ...
                            <?php
                                if(file_exists('./assets/storage/hubs.db')) {
                                    ...
                                    while(($hub = fgets($hubs_file)) !== false) {
                                        ...
                                        echo '<option value="'. $hub_details[1].'">'. ucfirst($hub_details[1]) .' </option>';
                                    }
                                }
                            ?>
